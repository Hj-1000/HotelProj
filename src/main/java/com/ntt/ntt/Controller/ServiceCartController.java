...
public class ServiceCartController {
    ...
    public ResponseEntity registerCart(@Valid ServiceCartItemDTO serviceCartItemDTO, BindingResult bindingResult, Integer memberId) {
        ...
        try {
            serviceCartItemId = serviceCartService.registerServiceCart(serviceCartItemDTO, memberId);
    ...
    public String orderHistory(Integer memberId ,Model model) {
        List<ServiceCartDetailDTO> serviceCartDetailDTOList =
                serviceCartService.listServiceCart(memberId);
    ...
    public ResponseEntity updateCartItem(@Valid ServiceCartItemDTO serviceCartItemDTO, BindingResult bindingResult, Integer memberId) {
        ...
        try {
            serviceCartItemService.updateServiceCartItemCount(serviceCartItemDTO, memberId);
    ...
    public ResponseEntity deleteCartItem(@PathVariable("serviceCartItemId") Integer serviceCartItemId, Integer memberId) {
        //내 장바구니가 맞는지 확인
        if (!serviceCartService.validateServiceCartItem(serviceCartItemId, memberId)) {
    ...
    public ResponseEntity orderServiceCartItem(@RequestBody ServiceCartOrderDTO serviceCartOrderDTO, BindingResult bindingResult, Integer memberId) {
        ...
        for(ServiceCartOrderDTO serviceCartOrders : serviceCartOrderDTOList) {
            if (!serviceCartService.validateServiceCartItem(serviceCartOrders.getServiceCartItemId(), memberId)) {
                return new ResponseEntity<String>("주문권한이 없습니다.", HttpStatus.FORBIDDEN);
            }
        }
        Integer serviceOrderId = serviceCartService.orderServiceCartItem(serviceCartOrderDTOList, memberId);
