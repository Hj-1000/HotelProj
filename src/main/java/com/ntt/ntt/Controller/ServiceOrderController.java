...
public class ServiceOrderController {
    ...
    public ResponseEntity order(@Valid ServiceOrderDTO serviceOrderDTO, BindingResult bindingResult, Integer memberId) {
        //만약에 아이템 id가 없다면
        //만약에 수량이 없다면
        //유효성 검사
        ...
        //먼저 저장을 한다
        Integer result =
                serviceOrderService.createOrder(serviceOrderDTO, memberId);
    ...
    public String orderHistory(@PathVariable("page") Optional<Integer> page,
        ...
        Page<ServiceOrderHistoryDTO> serviceOrderHistoryDTOPage =
                serviceOrderService.getOrderList(memberId, pageable);

        model.addAttribute("orders", serviceOrderHistoryDTOPage);
        //html 들어가서 getContent() 함수 호출할것임
    ...
    public ResponseEntity cnacelOrder(@PathVariable("serviceOrderId") Integer serviceOrderId, Integer memberId) {
        ...
        if (!serviceOrderService.validateOrder(serviceOrderId, memberId)) {
            //내 제품이 아니라면
